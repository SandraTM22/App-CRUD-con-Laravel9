    ...
    <div class="container mx-auto px-8 mt-6">
        <form action="{{route('Cursos.update',$post)}}" method="POST">
            ...
            <div class="w-full flex gap-4 justify-between flex-col mb-4">
                ...
                <textarea required class="capitalize border-2 border-double border-gray-500 rounded p-2" name="titulo" id="titulo" cols="30" rows="3">{{$post ->title}}</textarea>
            </div>
            <div class="w-full flex gap-4 justify-between flex-col mb-4">
                <label class="text-lg font-medium" for="body">Contenido</label>
                <textarea required class="capitalize border-2 border-double border-gray-500 rounded p-2" class="w-full" name="body" id="body" cols="30" rows="10">{{$post ->body}}</textarea>
            </div>
